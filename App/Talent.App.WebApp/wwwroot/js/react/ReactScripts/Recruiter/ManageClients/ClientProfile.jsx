...
class ClientProfileModal extends React.Component {
    ...
    loadData(id) {
        if (id != undefined) {
            ...
            $.ajax({
                url: 'https://talentprofilet.azurewebsites.net/profile/profile/getEmployerProfile?id=' + id + '&role=' + 'employer',
                headers: {
    ...
    saveData() {
        var cookies = Cookies.get('talentAuthToken');
        $.ajax({
            url: 'https://talentprofilet.azurewebsites.net/profile/profile/saveClientProfile',
            headers: {
                'Authorization': 'Bearer ' + cookies,
                'Content-Type': 'application/json'
            },
